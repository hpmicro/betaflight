...
int usbd_msc_sector_read(uint8_t busid, uint8_t lun, uint32_t sector, uint8_t *buffer, uint32_t length)
{
    ...
    if (g_sd.host->card_inserted) {
        ...
        hpm_stat_t status = sd_read_blocks(&g_sd, (uint8_t *) sys_buf_addr, sector, length / g_sd.block_size);
...
int usbd_msc_sector_write(uint8_t busid, uint8_t lun, uint32_t sector, uint8_t *buffer, uint32_t length)
{
    ...
    if (g_sd.host->card_inserted) {
        uint32_t sys_buf_addr = core_local_mem_to_sys_address(0, (uint32_t)buffer);
        hpm_stat_t status;
        status =  sd_write_blocks(&g_sd, (uint8_t *) sys_buf_addr, sector, length / g_sd.block_size);
        return status;
    } else {
        return -1;
    }
